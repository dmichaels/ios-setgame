public class TableDeck: Deck<TableCard> {
    ...
    public static func randomMagicSquare(simple: Bool = false) -> [TableCard] {
        ...
        magic.append(TableCard.matchingSetValue(magic[0], magic[1])); // [3] from [0] and [1]
        magic.append(TableCard.matchingSetValue(magic[0], magic[2])); // [4] from [0] and [2]
        magic.append(TableCard.matchingSetValue(magic[3], magic[4])); // [5] from [3] and [4]
        magic.append(TableCard.matchingSetValue(magic[2], magic[5])); // [6] from [2] and [5]
        magic.append(TableCard.matchingSetValue(magic[1], magic[5])); // [7] from [1] and [5)
        magic.append(TableCard.matchingSetValue(magic[0], magic[5])); // [8] from [0] and [5]
