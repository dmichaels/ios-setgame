...
extension Deck {
    ...
    ///
    public static func probabilityOfAtLeastOneSet(for ncards: Int) -> Double {
        guard (ncards >= 3) && (ncards <= 81) else { return 0.0 }
        guard (ncards < 21) else { return 1.0 } // Max no-SET ncards is 20
        guard let setless = setlessCounts[ncards] else {
            // We only stored counts up to 21; this shouldn't be hit given checks above.
